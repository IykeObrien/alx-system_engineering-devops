...
mysql> CREATE USER 'holberton user'@'localhost' IDENTIFIED BY 'projectcorrection280hbtn';

mysql> GRANT GRANT REPLICATION CLIENT ON *.* TO 'holberton user'@'localhost';

mysql> FLUSH PRIVILEGES;
Creating Database, Tables and adding Data to the Tables
mysql> CREATE DATABASE db name_;
...
mysql> INSERT INTO table name VALUES (val 1, val 2);

-- Verify if data was added succesfully do
mysql> SELECT col 1, col 2 FROM tb name;
Setting Up MySQL Replication
First create replication user and grant replication priviledge ( best practice).
mysql> CREATE USER 'replica user'@'%' IDENTIFIED BY 'replica user pwd';

mysql> GRANT REPLICATION SLAVE ON *.* TO 'replica user'@'%';

mysql> FLUSH PRIVILEGES;

-- to verify
mysql> SELECT user, Repl slave priv FROM mysql.user;
...
+------------------+----------+--------------+------------------+-------------------+
| File             | Position | Binlog Do DB | Binlog Ignore DB | Executed Gtid Set |
+------------------+----------+--------------+------------------+-------------------+
...
$ mysqldump -uroot -p db name > export db name.sql

$ scp -i _idenetity_file_ export_db name.sql user@machine ip:location
Then ssh to replica machine ip adress to import this tables to replica mysql-server
$ mysql -uroot -p 
password:
...
$ mysql -uroot p db name < export db name.sql
password:
